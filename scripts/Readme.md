...
# summarize function usage (raw ➜ alias map)
python scripts/analysis/function_stats.py -i Toucan-1.5M/Toucan-1.5M -o stats/function_stats_raw.csv --meta-output stats/function_stats_raw.json --alias-output stats/function_alias.json --workers 32
# obfuscate全量 jsonl，并基于混淆数据重建 stats/param_pool
...
# generate api options (available/params/param_values) from obfuscated jsonl
python scripts/build_has/build_has_api_script.py -i data/toucan_obf.jsonl -s stats/function_stats.json -o data/has_api_available.jsonl --mode available --negatives 9 --max-samples 200
python scripts/build_has/build_has_api_script.py -i data/toucan_obf.jsonl -s stats/function_stats.json -o data/has_api_param_values.jsonl --mode param_values --negatives 5 --max-samples 200 --param-pool stats/param_pool.json
# make jsonl file readable（同时展示混淆名 + 原名）
python scripts/analysis/pretty_toucan.py -i data/toucan_obf.jsonl -n 1 --alias-map stats/function_alias.json > data/toucan_readable.txt
